...
const MLabURL =
  "mongodb://jV5H6Gv8gEcs9Fwy:bAxrRtY8mrEZv59sW27a@ds159073.mlab.com:59073/drqp-nest";
...
app.get("/", function (req, res) {
  res.send(
    "Hi there! Open the Angular app in another browser window to view my content!"
  );
});
...
app.get("/api/bookmarks/:_id", function (req, res) {
  BookmarkData.find({ _id: req.params._id }, function (err, bookmarks) {
    if (err) {
      res.send(err);
    }
    res.json(bookmarks);
...
app.delete("/api/bookmarks/:id", function (req, res) {
  BookmarkData.deleteOne({ _id: req.params.id }, function (err) {});
});
